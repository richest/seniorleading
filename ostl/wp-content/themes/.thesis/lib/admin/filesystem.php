...
class thesis_asset_handler {
	public function __construct() {
		...
		add_action('admin_init', array($this, 'core_check'));
	...
	public function core_message($update_actions, $theme) {
		if ($theme == 'thesis')
			$update_actions =
				'<a href="'. wp_nonce_url(admin_url('admin.php?page=thesis'), 'thesis_did_update') .'">'. __('Return to Thesis!', 'thesis') .'</a>';
	...
	public function core_check() {
		...
		if (is_object($transient) && property_exists($transient, 'response') && isset($transient->response['thesis']) && property_exists($transient, 'checked') && isset($transient->checked['thesis']) && version_compare($transient->response['thesis']['new_version'], $transient->checked['thesis'], '<=')) {
			$all = array('thesis_callout', 'update_themes', 'thesis_skins_update', 'thesis_boxes_update', 'thesis_packages_update', 'thesis_core_update');
	...
	public function sitewide_nag() {
		...
		if (($data = get_transient('thesis_core_update')) && is_super_admin() && version_compare($thesis->version, $data['new_version'], '<') && $GLOBALS['pagenow'] !== 'update.php')
			echo
				"<div id=\"update-nag\">\n",
				"\t", sprintf(__('Thesis %s is available! %sUpdate Now!</a>', 'thesis'), esc_attr($data['new_version']), '<a onclick="if(!thesis_update_message()) return false;" id="thesis-update-link" href="'. esc_url(wp_nonce_url('update.php?action=upgrade-theme&amp;theme=thesis', 'upgrade-theme_thesis')) .'">'), "\n",
				"</div>\n";
	}
	
	public function reset_transients() {
		foreach (array('skins', 'boxes', 'packages', 'core') as $tr)
	...
	public function get_all_updates() {
		...
		if (empty($all))
			return;
		$from = apply_filters('thesis_dev_updates', false) ? 'http://dev.thesisapi.com/update.php' : 'http://thesisapi.com/update.php';
		$post_args = array(
			'body' => array(
				'data' => serialize($all),
...
if (class_exists('WP_Upgrader')): // wp-admin/includes/class-wp-upgrade.php has been included. safe to do things :)
	class thesis_update_objects extends WP_Upgrader {
		...
		public function do_updates() {
			...
			if (empty($_GET['update_type']) || ! in_array($_GET['update_type'], $types))
				...
				if ($this->raw_path === $dirs[$_GET['update_type']]) return; // bail if no folder given
				$run = array(
	...
	class thesis_update_objects_skin extends WP_Upgrader_Skin {
		public function footer() {
			if ($this->upgrader->complete_success) {
				...
				echo "<p><a href=\"", wp_nonce_url(admin_url("admin.php?page=thesis&canvas={$canvas}"), 'thesis_did_update'), "\">", __('Click here to finish!', 'thesis'), "</a></p>";
			}
			echo "</div>";
		}
	}
	...
	class thesis_generate extends WP_Upgrader {
		...
		private function create_seed() {
			...
			$entries = array('css', 'boxes', 'packages', 'vars', 'templates', '_design', '_display');
			$build = array();
